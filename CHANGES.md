...
its side of the `String`/`ByteString`/`Text` argument - two
representations, both correct - and gains `Control.Applicative`,
the blocker that stood between it and real parser code on GitHub.
Programs that only use ASCII see no change; programs that touch
Unicode see a great deal of it, all of it toward GHC.

**Strings and Text: two representations, on purpose
(docs/MANUAL.md ch. 14).** GHC spreads packed strings across
`Text` and `ByteString`, strict and lazy, plus `Builder`. AHC ships
exactly two representations and will not add a third: `String` =
`[Char]`, the Report's type, and one packed UTF-8 `Text` serving
both the text and the bytes role.

- **`String` is code-point correct end to end.** `Char` holds a
  code point; every byte stream holds UTF-8; the runtime has one
  encoder and one decoder between them. Previously every path that
  built a `String` produced one `Char` per *byte*, so
  `length "\955"` was 2 and any `Char` above 255 was truncated on
  output. Invalid UTF-8 on input decodes to U+FFFD rather than
  throwing - the one deliberate divergence from GHC, chosen so
  `getContents` over binary garbage cannot kill a program. NUL is
  data: literals and the buffer-reading IO paths carry lengths.
- **Literals cost nothing.** Each distinct literal in a unit is one
  static, deduplicated by content and built once, instead of being
  re-consed at every evaluation (`b_strings` 1.60s -> 1.27s).
- **`Data.Text`**: an `AHC_BYTES` slice - `{len, off, payload}` -
  with a code-point API over byte offsets, so `byteLength` and
  `take`/`drop` are O(1) shares of one immutable payload. Byte
  order over valid UTF-8 *is* code-point order, so `Eq` and `Ord`
  are one `memcmp`. The payload lives in the collector's new
  **atomic** allocation kind (kept alive, never scanned) - the
  owner kind the collector design note named long ago; bignum limbs
  moved there too, and were the live example of the false-retention
  hazard it removes.
- **`OverloadedStrings`, unconditional and free.** A literal is
  `IsString a => a`, desugared through `fromString` exactly as
  numbers go through `fromInteger`, with ambiguous literals
  defaulting to `[Char]` so every existing program means what it
  meant. `fromString` at a known instance is resolved when evidence
  is attached, so a `Text` literal compiles to a static packed
  payload and never packs at runtime.
- **`Text` crosses the C ABI** as `const char *` in every position
  (import, export, callback; arguments and results), normalized on
  the way in, with all five bindgen spokes mapping it to the host's
  native string. Interior NULs are refused with one clear error
  instead of being truncated differently by each host, and a NULL
  argument dies into the armed frame instead of faulting in
  `strlen`.
- **`Data.Char` is Unicode**, from tables generated by the oracle
  GHC itself (`tests/gen_unicode.hs`): all 1,114,112 code points
  agree across the eight classification and case functions. The
  fuzzer's literals now range over Unicode as a result.
- **`Semigroup` and `Monoid`** join the Prelude as source classes,
  with instances at `[a]`, `Text`, `Ordering`, `Maybe` and `()`.

An adversarial review of the finished milestone found seven defects
that every suite had passed over - surrogate escapes in literals,
`show` emitting decimal where the Report wants ASCII mnemonics, a
generic `showList` truncating at a raw NUL, and four FFI edges -
each now fixed and pinned.
