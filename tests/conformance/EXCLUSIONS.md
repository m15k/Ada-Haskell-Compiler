...
| lib | `Data.Array`/`Text.Read` are monomorphic subsets | `Ratio a` and `Complex a` are POLYMORPHIC as of the polish milestones (lib_poly_ratio_complex.hs); Array stays at Int indices (list-backed, O(n) reads) and Read at Int/Integer/Bool/lists/pairs |
